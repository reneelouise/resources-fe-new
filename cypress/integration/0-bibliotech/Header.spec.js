describe("Testing the heeader", () => {
  ...
  it("Login flow", () => {
    ...
    cy.get(".links").should("have.length", 3); // one navlink is showing
    cy.get(".NavLinks button").first().should("have.text", "Resources"); // nav link has the name resources
    ...
    cy.get(".Users-In-Dropdown")
      .last()
      .should("have.text", "Yann LeCun")
      .click(); //check that the last user in dropdown is Yann LeCun and select him
